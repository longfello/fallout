<?php
namespace Microsoft\MsAPI;

class OAuth
{
    private static $clientId = "26d8f72c-03f3-4e8f-a73c-4ac283b57728";
    private static $clientSecret = "fci3krmZ75hB0zYYBY0CwhO";
    ...
    public static function getAccessToken($redirectUri)
    {
        ...
        $current_token = $_SESSION['msapi_access_token'];
        if (!is_null($current_token)) {
            // Check expiration
            $expiration = $_SESSION['msapi_token_expires'];
            if ($expiration < time()) {
                error_log('Token expired! Refreshing...');
                // Token expired, refresh
                $refresh_token = $_SESSION['msapi_refresh_token'];
                $new_tokens = self::getTokenFromRefreshToken($refresh_token, $redirectUri);

                // Update the stored tokens and expiration
                $_SESSION['msapi_access_token'] = $new_tokens['access_token'];
                $_SESSION['msapi_refresh_token'] = $new_tokens['refresh_token'];
    ...
    {
        return (isset($_SESSION['msapi_access_token'])) ? !is_null($_SESSION['msapi_access_token']) : false;
    }

    public static function getRedirectUri($path = '/authorize.php')
    {
        return htmlentities('https://' . $_SERVER['SERVER_NAME'] . $path);
    }
}
