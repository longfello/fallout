...
class TbMenu extends TbBaseMenu {
	...
	const TYPE_BUTTONS = 'list button';
    ...
    if ($this->type == self::TYPE_BUTTONS) {
      foreach($this->items as $key => $item) {
        if (!isset($item['linkOptions'])) $item['linkOptions'] = array();
        $this->items[$key]['linkOptions']['class'] = 'btn btn-flat btn-default';
      }
//      $this->itemCssClass .= 'btn btn-flat btn-info';
    }
		...
		$validTypes = array(self::TYPE_TABS, self::TYPE_PILLS, self::TYPE_LIST, self::TYPE_NAVBAR, self::TYPE_BUTTONS);
