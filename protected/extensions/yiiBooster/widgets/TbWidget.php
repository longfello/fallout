...
abstract class TbWidget extends CWidget {
	...
	protected function isValidContext($context = false) {
		if($context)
			return defined($this->get_called_class52().'::CTX_'.strtoupper($context));
		else
			return defined($this->get_called_class52().'::CTX_'.strtoupper($this->context));
	}
	...
	protected function getContextClass($context = false) {
		if($context)
			return constant($this->get_called_class52().'::CTX_'.strtoupper($context).'_CLASS');
		else
			return constant($this->get_called_class52().'::CTX_'.strtoupper($this->context).'_CLASS');
	}

  function get_called_class52() {
    $arr = array();
