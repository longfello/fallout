...
function playersCount($id){
  $sql='SELECT COUNT(*) AS count_players FROM `reg_source` WHERE source='.$id;
...
function playersRef($data){
  $player=Players::model()->findByPk($data->id);
  ?>
  <div class="box box-default collapsed-box">
    <div class="box-header with-border">
      <h3 class="box-title"><?php echo $player->user; ?></h3>
...
$(document).on('click', '.show-level', function(e){
  if ($(this).children().hasClass('fa-plus')) {
      ...
      var self = this;
      var res=$.post( '/admin/statistic/loadAdv', {uid:uid}, function( data ) {
...
$(document).on('click','.show-player .fa-plus-circle', function(e){
  ...
  var level = $(this).data('level');
    var self = this;
  $.post( '/admin/statistic/loadPlayerAdv', {uid:uid,level:level}, function( data ) {
