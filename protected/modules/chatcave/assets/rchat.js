...
var chat = {
    ...
    init: function() {
        ...
        $('#chatForm').submit(function() {
            ...
            // Если игрок не в пещерах
            if (!chat.currentUserData.travel_place) {
                alert(rchatLang.notInCaves);
            ...
            $.post('/chatcave/ajax/SubmitForm', $(this).serialize() + '&RChat[message]=' + encodeURIComponent(sentMsg), function(r) {
                working = false;
        ...
        $('#ignore').click(function() {
            ...
            $.post('/chatcave/ajax/ignore', data, function(r) {
        ...
        $('#rchat').on('click', '.ignoreLine input', function() {
            ...
            $.post('/chatcave/ajax/DeleteIgnore', playerId, function(r) {
        ...
        $('#rchat').on('click', '.deleteMsg', function() {
            ...
            $.post('/chatcave/ajax/DeleteMessage', 'chat-id=' + chatLine.data('chat-id'), function() {
                chatLine.remove();
            });
        });
    ...
    render: function (template, params) {
        ...
        var deleteMsgIcon = function() {
            if (chat.currentUserData.rank == 'Админ' || chat.currentUserData.rank == 'Модер' || chat.currentUserData.rank == 'Чат-Модер') {
                return '<span class="deleteMsg" title="'+rchatLang.delete+'">&times; </span>'
            }
            else {
                return '';
            }
        ...
        switch(template) {
            ...
            case 'user':
                arr = [
                    ...
                    params.clanIcon, ' [<a href="/view.php?view=', params.id, '" target="_blank">', params.id, '</a>]</div>'
                ];
                break;
    ...
    getChats: function(callback) {
        $.get('/chatcave/ajax/GetChats', {lastId: chat.data.lastId}, function(r) {
    ...
    getUsers : function(callback) {
        $.get('/chatcave/ajax/GetUsers', {}, function(r) {
    ...
    autoGetCurrentUser: function() {
        (function getCurrentUsersTimeoutFunction(){
            ...
            $.get('/chatcave/ajax/GetCurrentUser', {}, function(r) {
    ...
    checkIgnore: function() {
        ...
        $.post('/chatcave/ajax/CheckIgnore', data, function(r) {
            chat.ignoreData = r;
    ...
    addIgnoreMessage: function() {
        $.get('/chatcave/ajax/GetCurrentTime', {}, function(r) {
    ...
    getIgnorePlayers: function() {
        $.get('/chatcave/ajax/GetIgnorePlayers', {}, function(r) {
