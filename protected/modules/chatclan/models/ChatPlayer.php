...
class ChatPlayer extends CActiveRecord
{
    ...
    public static function getUsers()
    {
        $players = Yii::app()->db->createCommand()
            ...
            ->from('players')
            ->join('{{chat_player_clan}} AS cp', 'cp.player_id = id')
            ->join('{{chat_avatar}} AS ca', 'ca.player_id = cp.player_id')
            ->join('{{chat_avatar_base}} AS cab', 'cab.avatar_id = ca.avatar_id')
            ->where('TIMESTAMPDIFF(SECOND, cp.last_activity, NOW()) <= 30')
            ->andWhere('clan = ' . Yii::app()->stat->clan)
            ->order('id')
        ...
        foreach ($players as $player)
        {
            $playerData[] = array(
                ...
                'style' => Extra::getChatCssColorUser($player->rank),
