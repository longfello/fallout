...
class AjaxController extends Controller
{
    ...
    public function actionGetChats()
    {
        ...
        $cmd->from('{{chat_clan}} c');
        $cmd->join('players AS p', 'p.id = c.player_id');
        $cmd->leftJoin('players AS pp', 'pp.id = c.to_player');
        $cmd->order('c.id DESC');
        $cmd->where('c.id > :id', array(':id' => $lastId));
        $cmd->andWhere('c.clan = :clan', array(':clan' => Yii::app()->stat->clan));
        ...
        echo CJSON::encode(array('chats' => $chats));
    }
    ...
    public function actionGetUsers()
    {
        ...
        ChatPlayer::model()->updateByPk(Yii::app()->stat->id, array('last_activity' => new CDbExpression('NOW()')));

        // Получить всех активных в чате игроков в течении последних 30 секунд
        $users = ChatPlayer::getUsers();
    ...
    public function actionGetCurrentUser()
    {
        ...
        $criteria->select = 'id, user, rank, clan';
