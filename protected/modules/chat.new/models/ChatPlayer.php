...
class ChatPlayer extends CActiveRecord
{
    ...
    public function rules()
    {
        ...
        return array(
            ...
            array('player_id, lang_slug, last_activity', 'safe', 'on'=>'search'),
        );
    }
     ...
     */
    public static function getUsers($lang_slug, $withoutPlayer = false, $appendSelf = true)
    {
	    $ids = Players::getOnline('chat');
	    if ($appendSelf) {
		    $ids[] = Yii::app()->stat->id;
	    }
	    ...
	    $players = Yii::app()->db->createCommand()
            ...
            ->from('players')
            ->join('{{chat_player}} AS cp', 'cp.player_id = id')
            ->join('{{chat_avatar}} AS ca', 'ca.player_id = cp.player_id')
            ->join('{{chat_avatar_base}} AS cab', 'cab.avatar_id = ca.avatar_id')
//            ->andWhere('cp.last_activity >= NOW() - 90')
            ->andWhere('players.id IN ('.implode(',', $ids).')')
...
//            /** @var $player Players */
		        $playerData[$player->id] = array(
			        ...
			        'updateChat' => isset($player->update_chat)?$player->update_chat:null,
			        'avatar' => self::getAvatar($player->avatar, Ban::getBanTime(Ban::BAN_CHAT, $player))
		        );
	        }
        }
    ...
    public static function getRenderPlayerData($player_id){
