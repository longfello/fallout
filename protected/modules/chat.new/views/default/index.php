...
<div class="chat-module" data-ban="<?= Ban::check(Ban::BAN_CHAT, false)?Ban::getBanTime(Ban::BAN_CHAT):0 ?>" data-last-id="<?= max( array_keys( $messages ) ) ?>">
    <div class="chat-header">
        <ul class="chat-nav">
            <?php if (Yii::app()->stat->clan): ?>
            <li>
                <a href="/chatclan">
                    <span><?= t::get('Чат клана') ?></span>
                </a>
            </li>
            <? endif; ?>
            <li>
                <a href="/radio.php?back=chat">
                    <span><?= t::get('Радио') ?></span>
                </a>
            </li>
            <li>
                <a href="<?= t::get_forum_link(); ?>viewtopic.php?f=2&t=2" title="<?= t::encHtml('Правила чата') ?>">
                    <span><?= t::get('Правила') ?></span>
                </a>
            </li>
            <li>
                <a href="#you-ignore" class="btnIgnore">
                    <span><?= t::get('Игнор') ?></span>
                </a>
            </li>
            <li>
                <a href="#show-player" class="btnPlayerList active">
                    <span><?= t::get('Игроки') ?></span>
                </a>
            </li>
        </ul>
        <div id="you-ignore" class="you-ignore">
            <p><?= t::get("У вас в игноре <span class='number' id='ignoreCount'>%s</span> человек(а):", array(0)) ?><span id="ignorePlayers"></span></p>
            <p id="ignoreList"><span class="ignoreLine"></span></p>
        </div>

        <? $form = $this->beginWidget('CActiveForm', array(
            'id' => 'chatForm',
            'htmlOptions' => array('autocomplete' => 'off', 'class' => 'write-to-chat')
        )); ?>
        <img src="/images/smilies/ag.gif" alt="" id="showSmileBtn">
        <?= $form->hiddenField($model, 'message')  ?>
        <div class="write-message" id="write-message" contenteditable="true"></div>
        <?= CHtml::submitButton(t::get('Отправить'), array('class' => 'send-message')); ?><br />
...
<!--                <img name="17" img src="/images/smilies/aq.gif" border="0" alt="*DIABLO*">-->
    ...
    <div class="chat-main">
        <div class="chat-wrapper">
            <div class="another-one-wrapper">
                <div class="chat-container">
                    <div class="chat-text">
                        <ul>
                            <?php
                            foreach($messages as $message) {
                                $this->renderPartial('__message', array('message' => $message));
                            }
                            ?>
                        </ul>
                    </div>
                </div>
                <div id="show-player" class="show-player open">
                    <h2><?= t::get('Список игроков') ?></h2>
                    <div class="wrapper-players-list">
                        <ul><?= $players ?></ul>
                    </div>
                </div>
            </div>
...
<div class="hidden">
    <div class="radio">
        <div id="volnorez_live_player" style="width:450px;height:27px;display:block;color:#33cc00"><br /><?= t::get('Слушать') ?> <a href="//volnorez.com/radio-revival-online" title="<?= t::encHtml('радио Radio Revival Online') ?>"><?= t::get('радио Radio Revival Online') ?></a><br /><script type="text/javascript">var jscode_player_id="00133e35";var jscode_options="1f";var cShareBtnMode="local";var cSkinURL_player="http://volnorez.com/application/maxsite/plugins/wcdesign/skins/liveplayer_skin_modern.swf";var cSkinParams_player="clr_skin:0,clr_text:33cc00,clr_btn:33cc00,clr_btn_h:ccff00,clr_btn_d:ccff00,clr_volume:ccff00";</script><br /><script type="text/javascript" src="http://volnorez.com/application/maxsite/plugins/jscode/js/jscode_player.js"></script><br /></div>
    </div>
</div>
...
$assets_path = Yii::app()->assetManager->publish( Yii::getPathOfAlias( 'chat' ) . '/assets' );
Yii::app()->clientScript->registerScriptFile( Yii::app()->assetManager->publish(Yii::getPathOfAlias( 'webroot' ) . "/js/i18n/" . t::iso() . "/lang.js"));
Yii::app()->clientScript->registerScriptFile( $assets_path . '/js/kchat.js' );
Yii::app()->clientScript->registerScriptFile( $assets_path . '/js/jquery.mCustomScrollbar.concat.min.js' );
Yii::app()->clientScript->registerCssFile( $assets_path . '/css/rchat.css' );
Yii::app()->clientScript->registerCssFile( $assets_path . '/css/jquery.mCustomScrollbar.css' );
